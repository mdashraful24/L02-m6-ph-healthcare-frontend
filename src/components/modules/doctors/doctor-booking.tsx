...
export default function DoctorBooking({ doctorId }: { doctorId: string }) {
    ...
    return (
        <>
            <div>
                {schedules.map((schedule) => (
                    <div key={schedule.id} className="flex flex-col gap-3 border p-4 rounded-md">
                        ...
                        {isBookingOpen(schedule.startDateTime) ? (
                            <Button size="lg" className="w-full" disabled={bookPending} onClick={() => handleBooking(schedule)}>
            ...
            <Dialog open={!!confirmation} onOpenChange={(open) => {
                if (!open) {
                ...
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>Please pay within 10 minutes to keep your booking.</DialogDescription>
                        <span>Data and Time: {" "} {confirmation ? format(confirmation?.schedule?.startDateTime!, "PPPP") : "N/A"}</span>
                    </DialogHeader>
                    <DialogFooter>
                        <Button variant="outline" onClick={() => setConfirmation(null)}>Pay Later</Button>
                        <Button onClick={() => {
